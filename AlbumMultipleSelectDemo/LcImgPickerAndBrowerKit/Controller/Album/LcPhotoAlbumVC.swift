...
class LcPhotoAlbumVC: LcBaseVC, PHPhotoLibraryChangeObserver, UICollectionViewDelegate, UICollectionViewDataSource {
        ...
        private func getAllPhotos() {
            ...
            if albumAccessLimited() == true {return;}
        ...
        private func albumAccessLimited() -> Bool{
            /**注意点！！-这里必须注册通知，不然第一次运行程序时获取不到图片，以后运行会正常显示。体验方式：每次运行项目时修改一下 Bundle Identifier，就可以看到效果。*/
        ...
        private func selectSuccess(fromeView: UIView, selectAssetArray: [PHAsset]) {
            ...
            for idx in 0 ..< selectAssetArray.count {
                let photoModel = LcPhotoModel()
                
                
                let asset = selectAssetArray[idx]
                
                if asset.mediaType == PHAssetMediaType.video {//此选中项是视频
                    group.enter()
                    ...
                    manager.requestAVAsset(forVideo: asset, options: options) { (asset:AVAsset?, audioMix:AVAudioMix?,info:[AnyHashable:Any]?) in
                        ...
                        photoModel.videoLocalAddress = urlUse
                        group.leave()
                    }
        ...
        func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            ...
            cell.giveContent(seconds: Int(asset.duration))
            ...
            if selectStyle == .number {
                if let Index = photoData.seletedAssetArray.firstIndex(of: asset) {
                    cell.layer.mask = nil
                     ...
                     */
                    cell.selectButton.isSelected = false
                    if maxSelectCount != 0,(photoData.seletedAssetArray.count >= maxSelectCount){//(asset.mediaType == PHAssetMediaType.video && self.isFirstSelectPhoto == true) ||
                        let maskLayer = CALayer()
                        maskLayer.frame = cell.bounds
                        maskLayer.backgroundColor = UIColor.init(white: 1, alpha: 0.5).cgColor
                        cell.layer.mask = maskLayer
